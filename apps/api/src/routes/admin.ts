...
adminRoutes.get('/pages/:id', async(c)=>{const pageId=c.req.param('id');const page=await c.env.DB.prepare('SELECT * FROM pages WHERE id=?').bind(pageId).first();const localizations=await c.env.DB.prepare('SELECT * FROM page_localizations WHERE page_id=?').bind(pageId).all();const sections=await c.env.DB.prepare('SELECT * FROM page_sections WHERE page_id=? ORDER BY position').bind(pageId).all();return c.json({page,localizations:localizations.results,sections:sections.results});});
