...
publicRoutes.get('/site', async (c) => {
  ...
  if (!page) return c.json({page:null,sections:[],events:[],settings:{}});
  const [sections,events,settingsRows] = await Promise.all([
    ...
    c.env.DB.prepare("SELECT key,value_json FROM site_settings WHERE key IN ('event_popup','social','general')").all<any>()
  ]);
  const settings:Record<string,unknown>={};
  for(const row of settingsRows.results){try{settings[row.key]=JSON.parse(row.value_json)}catch{settings[row.key]={}}}
  return c.json({page,sections:sections.results,events:events.results,settings},200,{'cache-control':'public,max-age=30,s-maxage=60'});
});
